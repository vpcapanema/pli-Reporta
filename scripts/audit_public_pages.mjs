...
const PAGES = [
  {
    ...
    deps: ['js/db.js', 'js/api.js', 'js/camera.js', 'js/map-pick.js', 'js/map-style.js'],
  },
  {
    route: '/mapa',
    file: 'viewer.html',
...
for (const page of PAGES) {
  ...
  for (const dep of [page.script, ...page.deps]) {
    ...
    if (err) issues.push(`${title}: erro de sintaxe em ${dep}: ${err.split('\n')[0]}`);
  }
}

// Páginas estáticas extras em /static/
for (const extra of ['traffic-icons-preview.html', 'traffic-icons-guide.html']) {
  const abs = path.join(frontend, extra);
  if (!fs.existsSync(abs)) continue;
  const html = fs.readFileSync(abs, 'utf8');
  const buttons = [...html.matchAll(/<button[^>]*id="([^"]+)"/g)].map((m) => m[1]);
  if (buttons.length) {
    issues.push(`/static/${extra}: botões com id (${buttons.join(', ')}) sem script associado — revisar manualmente`);
  }
}
...
for (const page of PAGES) {
  console.log(`  ${page.route}: ${page.buttons.length} botões, script ${page.script}`);
}
