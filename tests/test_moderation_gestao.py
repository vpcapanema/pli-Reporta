...
def test_moderation_catalog(app_client):
    ...
    r = app_client.get("/api/v1/moderation/catalog", headers=headers)
    assert r.status_code == 200
    body = r.json()
...
def test_moderation_stats(app_client):
    headers = _login(app_client)
    r = app_client.get("/api/v1/moderation/stats", headers=headers)
    assert r.status_code == 200
    body = r.json()
    assert "fila" in body
...
def test_moderation_policy_get_and_patch(app_client):
    headers = _login(app_client)
    r = app_client.get("/api/v1/moderation/policy", headers=headers)
    assert r.status_code == 200
    policy = r.json()
...
def test_moderation_policy_simulate(app_client):
    headers = _login(app_client)
    r = app_client.post("/api/v1/moderation/policy/simulate?days=7", headers=headers)
    assert r.status_code == 200
    body = r.json()
    assert "total" in body
...
def test_moderation_reports_and_geojson(app_client, jpeg_bytes):
    ...
    r = app_client.get("/api/v1/moderation/reports", headers=headers)
    assert r.status_code == 200
    assert r.json()["total"] >= 1

    geo = app_client.get("/api/v1/moderation/reports.geojson", headers=headers)
